      ...
      case "scp":
        if (message.content.toLowerCase().includes("scp")) {
          ...
          for (i = 0; i < listOfNumbers.length; i++) {
            ...
            if (UrlExists(url)) {
              var scpQuery = `SELECT * FROM Scps WHERE Number = ${parseInt(
                listOfNumbers[i]
              )}`;
              ...
              if (queryResult[0]) {
                ...
                async function sendMsg(l = null, rpl = null) {
                  ...
                  const descId =
                    message.member.user.username.replace(" ", "") + makeid(50);
                  const contId =
                    message.member.user.username.replace(" ", "") + makeid(50);
                  const NotesId =
                    message.member.user.username.replace(" ", "") + makeid(50);
                ...
                async function tagsCollected(i, reply) {
                  ...
                  for (p = 0; p < itemTags.length; p++) {
                    if (itemTags[p]) {
                      if (p == 0)
                        embed.addField("Object Class", itemTags[p], true);
                      else
                        embed.addField("Object Tag", itemTags[p], true);
                    }
                  }
                  ...
                  .then(async function (html) {
                    ...
                    if (!fs.existsSync(`images/${itemClass}.png`)) {
                      ...
                      v.getPalette((err, palette) => {
                        itemClassColor = palette.Vibrant.hex;
                        var text = [""];
                        new DataReader("images/server.txt", {
                          encoding: "utf8",
                        })
                          .on("error", function (error) {
                            console.log("error: " + error);
                    ...
                    async function sendMsg(l = null, rpl = null) {
                      ...
                      if (itemClassImg)
                        originalEmbed.setThumbnail(itemClassImg);
                      ...
                      if (bigCont)
                        originalRow.addComponents(
                          new MessageButton()
                            .setCustomId(contId)
                            .setLabel(
                              "View Full Special Containment Procedures"
                            )
                            .setStyle("PRIMARY")
                        );
                      ...
                      if (collector) {
                        ...
                        collector.on("end", async (collected, reason) => {
                          ...
                          if (
                            !reason.includes("Found Interaction: ") &&
                            replyy
                          ) {
                            await reply.edit({ components: [] });
                          }
                    ...
                    async function descriptionCollected(i, reply, pageNo = 1) {
                      var chunks =
                        fullItemDescriptionSegmnt.match(/.{1,4050}/g);
                      //console.log(itemNo, fullItemDescriptionSegmnt.length, chunks.length)
